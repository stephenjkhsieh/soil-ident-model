...
def validate(
    ...
    # --- 新增：用來收集整個 Validation Set 的預測與真實標籤 ---
    all_preds = []
    all_targets = []
    ...
    # --- 新增：計算全局進階指標 (Macro F1, Precision, Recall) ---
    ...
    # 只在主進程 (Primary) 打印日誌與上傳 WandB
    if utils.is_primary(args):
        log_name = 'Test' + log_suffix
        _logger.info(
            f'{log_name}: [{last_idx+1}/{last_idx+1}]  '
            f'Time: {batch_time_m.val:.3f} ({batch_time_m.avg:.3f})  '
            f'Loss: {losses_m.val:>7.3f} ({losses_m.avg:>6.3f})  '
            f'Acc@1: {top1_m.val:>7.3f} ({top1_m.avg:>7.3f})  '
            f'Macro_F1: {macro_f1:>7.4f}  '
            f'Macro_Prec: {macro_prec:>7.4f}  '
            f'Macro_Rec: {macro_rec:>7.4f}'
        )

        # 呼叫 wandb 記錄進階指標與 Confusion Matrix
        if args.log_wandb and has_wandb:
            # 對應你的資料夾 B1 到 B6
            class_names = ['B1', 'B2', 'B3', 'B4', 'B5', 'B6']
            
            wandb.log({
                f"eval{log_suffix}/loss": losses_m.avg,
                f"eval{log_suffix}/top1": top1_m.avg,
                f"eval{log_suffix}/macro_f1": macro_f1,
                f"eval{log_suffix}/macro_precision": macro_prec,
                f"eval{log_suffix}/macro_recall": macro_rec,
                f"eval{log_suffix}/confusion_matrix": wandb.plot.confusion_matrix(
                    probs=None,
                    y_true=all_targets,
                    preds=all_preds,
    ...
    # --- 關鍵修改：將 macro_f1 放入回傳的 metrics 中 ---
    # timm 的 CheckpointSaver 會自動抓取你在 YAML 設定的 eval_metric (macro_f1) 來決定是否儲存最佳模型
    metrics = OrderedDict([
        ('loss', losses_m.avg), 
        ('top1', top1_m.avg), 
        ('macro_f1', macro_f1)
